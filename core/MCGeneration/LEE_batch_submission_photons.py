...
for ijob in xrange(njobs):
    ...
    cmd_input_name = "g_LEE_%03d.cmd" % ijob
    fcl_input_name = "g_prodtext_and_scanner_%03d.fcl" % ijob
    input_txt_file = "/a/data/morningside/kaleko/condor_LEE_output/single_photons/input_txt_files/%s" % text_input_name
    ...
    if os.path.isfile(input_txt_file):
        ...
        os.system("sed 's/XXXFCLFILENAMEXXX/%s/g' /a/home/houston/kaleko/condor/condor_LArLight/LEE_stuff/larlight_LEE_study_template_photons.cmd > %s" % (fcl_input_name, custom_cmd_file ))
