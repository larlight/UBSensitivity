...
if not 'CORE_DIR' in os.environ or not 'USER_MODULE' in os.environ:
    ...
    print ("\033[93m" + "Specified " + os.environ['CORE_RELPATH'] + " packages to be compiled by a user:"  + "\033[0m")
    valid_packages=''
    not_found=[]
    for package in packages:

        if os.path.isdir('%s/%s' % (os.environ['CORE_DIR'],package)):
...
fout=open('%s/GNUmakefile' % os.environ['CORE_DIR'],'w')
fout.write(makefile)
fout.close()
